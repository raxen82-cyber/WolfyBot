    ...
    if channel:
        ...
        if stored_message_id:
            try:
                ...
                logging.error(f"Non ho i permessi per modificare il messaggio in {channel.name} in {guild.name}")
        else:
            new_message = await channel.send(embed=embed)
            summary_message_ids[guild.id] = new_message.id
    ...
    if channel:
        ...
        if stored_message_id:
            try:
                ...
                logging.info(f"Cancellati i messaggi di attività precedenti a {stored_message_id} nel canale {channel.name} in {guild.name}")
            except discord.NotFound:
                logging.warning(f"Il messaggio di riferimento {stored_message_id} non è stato trovato in {channel.name} in {guild.name}")
            except discord.errors.Forbidden:
                logging.error(f"Non ho i permessi per cancellare i messaggi in {channel.name} in {guild.name}")
        else:
            logging.info(f"Nessun messaggio di attività di riferimento trovato per {guild.name}.")

async def clear_channel(channel):
    try:
        await channel.purge(limit=None)
        logging.info(f"Canale '{channel.name}' pulito.")
    except discord.errors.Forbidden:
        logging.error(f"Non ho i permessi per pulire il canale '{channel.name}'.")
    except discord.errors.NotFound:
        logging.warning(f"Il canale '{channel.name}' non è stato trovato.")
...
# RIMOSSA app = Flask(__name__) e le relative route/funzioni
...
async def on_ready():
    ...
    for g in bot.guilds:
        await send_summary(g, initial=True)
...
if __name__ == '__main__':
    ...
    if TOKEN:
        ...
        logging.error("Errore: La variabile d'ambiente DISCORD_TOKEN non è impostata.")
